...
class FriendsViewModel extends ChangeNotifier {
  ...
  FriendSearchRelation relationToSearchUser(String targetUserId) {
    final target = targetUserId.trim();
    if (target.isEmpty) return FriendSearchRelation.none;

    for (final f in _friends) {
      if (sameUserId(_extractUserId(f.requester), target) ||
          sameUserId(_extractUserId(f.recipient), target)) {
        return FriendSearchRelation.friends;
      }
    }
    ...
    for (final f in _pendingRequests) {
      ...
      if (sameUserId(_extractUserId(f.recipient), target)) {
  ...
  String? incomingFriendshipIdForUser(String targetUserId) {
    final target = targetUserId.trim();
    for (final f in _pendingRequests) {
      if (sameUserId(_extractUserId(f.requester), target)) {
        return f.id;
  ...
  String? _extractUserId(dynamic user) {
    if (user is FriendUser) return user.id;
