...
class _ScouterPlayersTabState extends State<ScouterPlayersTab> {
  ...
  PlayerDetail _entryToPlayer(LeaderboardEntry entry) {
    final userMap = entry.raw['user'] is Map
        ? Map<String, dynamic>.from(entry.raw['user'] as Map)
        : null;
    return PlayerDetail.fromJson({
      ...
      'elo': entry.elo,
      'tier': entry.tier,
      'rank': entry.raw['rank'] ?? entry.tier,
      'isPro': entry.raw['isPro'] ?? false,
      'team': entry.raw['team'],
    });
  }
        ...
        Navigator.push(
          ...
          MaterialPageRoute(
            builder: (_) => ScouterPlayerDetailScreen(
              playerUserId: entry.id.isNotEmpty ? entry.id : userId,
              scouterId: widget.scouterId,
              initialPlayer: _entryToPlayer(entry),
            ),
          ),
        );
      },
  ...
  Widget _leaderboardRow(
    ...
    return GestureDetector(
      onTap: () {
        final userId = entry.user?.id ?? '';
        if (userId.isNotEmpty || entry.id.isNotEmpty) {
          Navigator.push(
            context,
            MaterialPageRoute(
              builder: (_) => ScouterPlayerDetailScreen(
