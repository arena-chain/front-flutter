...
class ScouterWatchlistTab extends StatelessWidget {
  ...
  Widget _prospectCard(BuildContext context, ProspectStatus p,
    ...
    final displayLevel = p.prospectLevel == 'ELITE_PROSPECT'
        ? 'ELITE'
        : p.prospectLevel;
    final displayName = (p.playerNickname != null && p.playerNickname!.isNotEmpty && p.playerNickname != 'Player')
        ? p.playerNickname!
        : (p.playerId.length > 8 ? '${p.playerId.substring(0, 8)}...' : p.playerId);
    ...
    return Material(
      ...
      child: InkWell(
        onTap: () {
          final navId = p.playerId.isNotEmpty ? p.playerId : p.id;
          if (navId.isEmpty) return;
          final initialPlayerData = PlayerDetail.fromJson({
            '_id': navId,
            'userId': navId,
            'nickname': p.playerNickname ?? displayName,
          });
